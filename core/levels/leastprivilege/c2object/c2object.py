...
def create():
    ...
    start_message = (
        f'Find the minimum privilage to list a bucket and access function c2-func-access-{nonce} to check if you have the correct answer')
    levels.write_start_info(
        LEVEL_PATH, start_message, file_name='', file_content='')
    print(f'Step 1.Please use cmd below to update functions and get http trigger url\n gcloud functions deploy c2-func-access-{nonce} --source=core/levels/leastprivilege/c2object/functionaccess --allow-unauthenticated \n gcloud functions deploy c2-func-check-{nonce} --source=core/levels/leastprivilege/c2object/functioncheck --allow-unauthenticated ')
    
    print(f'Step 2.Use cmd below to check iam permissions of c2_access \n gcloud iam roles update c2_access_role_{nonce} --project={project_id} --permissions=permission1,permission2\n OR \n gcloud functions call c2-func-check-{nonce} --data \'{{\"permissions\":[\"permission1\",\"permission2\"]}}\' \n OR \n append ?permissions=permission1,permission2 after function url generated in Step 1 ')

    print(f'Step 3.Call c2-func-access-{nonce} with cmd \n gcloud functions call c2-func-access-{nonce} \n OR \n through function url generated in Step 1  \n') 
    
    print(f'Use function below to access level \n') 
    print(f'https://{FUNCTION_LOCATION}-{project_id}.cloudfunctions.net/c2-func-access-{nonce}') 
